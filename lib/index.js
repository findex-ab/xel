...
import { isArray, isFunction, isHTMLElement, isNumber, isString, isSvgTag, isUndefined, } from "./utils/is";
...
var setAttribute = function (x, el, key, value) {
    ...
    if (key === "stylesheet" && isHTMLElement(el)) {
        ...
        x.config.children = __spreadArray(__spreadArray([], (x.config.children || []), true), [
            X("style", {
                type: "text/css",
...
export var X = function (tag, config) {
    ...
    var genUid = function () {
        ...
        var h = 0;
        for (var _i = 0, _a = Array.from(tag + (err.stack ? err.stack : '')); _i < _a.length; _i++) {
            var c = _a[_i];
...
export var xUpdate = function (x, callee) {
    ...
    if (isHTMLElement(x.el) && isHTMLElement(next)) {
        var a = x.el.outerHTML;
        var b = next.outerHTML;
        if (a === b)
            return x.el;
    }
    var didUpdate = false;
    if (callee && !isUndefined(callee.nextValue) && !isUndefined(callee.oldValue) && isHTMLElement(next) && isHTMLElement(x.el) && next.children.length === x.el.children.length) {
        var len = Math.max(next.children.length, x.el.children.length);
        for (var i = 0; i < len; i++) {
            var a = x.el.children[i % x.el.children.length];
            var b = next.children[i % next.children.length];
